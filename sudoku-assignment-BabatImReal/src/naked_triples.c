...
int naked_triple(SudokuBoard *p_board)
{
    ...
    int duplicate = 0;
    for (int i = 0; i< BOARD_SIZE; i++){
        find_naked_triple(p_board->p_cols[i], p_naked_triple, &p_counter, &duplicate);
        find_naked_triple(p_board->p_rows[i], p_naked_triple, &p_counter, &duplicate);
        find_naked_triple(p_board->p_boxes[i], p_naked_triple, &p_counter, &duplicate);
    }
