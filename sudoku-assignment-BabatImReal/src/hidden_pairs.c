...
void find_hidden_double(Cell **p_cells, HiddenDouble *p_hidden_double,
                        int *p_counter)
{       
            ...
            for (int j = i+1; j < num_value; j++){
                if(is_hidden_pair(p_cells, hidden_double_value_array[i], hidden_double_value_array[j])){
                    for (int k = 0; k < BOARD_SIZE; k++){
                        ...
                        p_hidden_double[(*p_counter)++] = hidden_double_object;
                        }                    
                    }
                }
            }
...
{
    HiddenDouble p_hidden_double[BOARD_SIZE*BOARD_SIZE];
    int p_counter = 0;
    for (int i = 0; i< BOARD_SIZE; i++){
